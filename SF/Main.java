...
public class Main {
    ...
    private static Map<String, String[]> indexSortedArray;
    ...
    private static String[] doRename(String curr[], List<Integer> listRename) {
        List<Integer> renameAndSort = new ArrayList<>(listRename);
        Collections.sort(renameAndSort);
        String key = "";

        for (int i = 0; i < renameAndSort.size(); i++) {
            key += String.valueOf(renameAndSort.get(i));

            if (i < (renameAndSort.size() - 1)) {
                key += ",";
            }
        }

        if (indexSortedArray.containsKey(key)) {
            return indexSortedArray.get(key);
        }

        String temp[] = curr.clone();
        for (int i = 0; i < listRename.size(); i++) {
            int cur = listRename.get(i);
            temp[cur] = rename(curr[cur]);
        }

        String sortedArr[] = sort(temp);
        indexSortedArray.put(key, sortedArr);
